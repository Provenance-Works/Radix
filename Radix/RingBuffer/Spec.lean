...
theorem push_preserves_invariant (s : RingBufferState) (val : UInt8)
    (hinv : s.invariant)
    (hResult : RingBufferState.pushSpec s val = some s') :
    s'.invariant := by
  simp [RingBufferState.pushSpec] at hResult
  split at hResult
  · contradiction
  · simp at hResult
    subst hResult
    simp [RingBufferState.invariant, RingBufferState.count] at *
    omega

/-- `pop` preserves the invariant. -/
theorem pop_preserves_invariant (s : RingBufferState) (hinv : s.invariant)
    (hResult : RingBufferState.popSpec s = some (v, s')) :
    s'.invariant := by
  simp [RingBufferState.popSpec] at hResult
  split at hResult
  · contradiction
  · simp at hResult
...
theorem push_pop_empty (cap : Nat) (val : UInt8) (hcap : cap > 0) :
    let s := RingBufferState.empty cap
    match RingBufferState.pushSpec s val with
    | some s' =>
      match RingBufferState.popSpec s' with
      | some (v, _) => v = val
      | none => False
    | none => False := by
  simp [RingBufferState.empty, RingBufferState.pushSpec, RingBufferState.isFull,
        RingBufferState.count, RingBufferState.popSpec]
  omega

/-- `pop` after `push` on non-full buffer yields FIFO ordering:
    the first `pop` returns the oldest element, not the just-pushed one. -/
theorem push_preserves_fifo (s : RingBufferState) (val : UInt8)
    (hinv : s.invariant) (hne : ¬s.isEmpty) (hnf : ¬s.isFull)
    (hpush : RingBufferState.pushSpec s val = some s') :
    match RingBufferState.popSpec s' with
    | some (v, _) => v = s.contents.head (by
        simp [RingBufferState.isEmpty] at hne
        exact List.ne_nil_of_length_pos (by
          simp at hne; exact Nat.pos_of_ne_zero (List.length_ne_zero.mpr hne)))
    | none => False := by
  simp [RingBufferState.pushSpec, hnf] at hpush
  subst hpush
  simp [RingBufferState.popSpec, RingBufferState.isEmpty] at hne ⊢
  match s.contents, hne with
  | hd :: tl, _ => simp
